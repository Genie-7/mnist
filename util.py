...
def get_graph_from_image(image,desired_nodes=75):
    ...
    for node in nodes:
        if nodes[node]["rgb_list"]:
            ...
            nodes[node]["rgb_list"] = np.array([])  # Or handle the empty list case as you see fit
            G.add_node(node, features=np.zeros(NUM_FEATURES))
        if "features" not in G.nodes[node]:
                G.nodes[node]["features"] = np.zeros(NUM_FEATURES)
